...
\emph{Availability against use} rests on the widest spread of distinct first authors in the corpus, the decodability rows drawing on \Fdecgrpmin{} to \Fdecgrpmax{} distinct first authors over four to seven modalities, but on records that almost never test selectivity, so what is established is that a readout recovers the target, not that the model uses it. \emph{Acquisition and site structure} is reported
recoverable in every record that tests it but one (F2b, F3a, F3c), by many groups and in several
modalities, but not the best controlled: a comparison model appears in a minority of the records carrying F3a and F3c, and one
...
hypothesis. One qualification applies to all five: \Pctpreprint\% of the corpus is preprints, and the appraisal is the reporting audit of Fig.~\ref{fig:findings} rather than a certainty grade. Three consequences for a reader who studies or
