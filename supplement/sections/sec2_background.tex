...
several surveys~\cite{wan2025survey}; the properties they are sometimes used to separate --- faithfulness,
plausibility and intrinsic transparency --- are distinguished explicitly where they matter
...
hidden state before the unembedding; its logits and tokens are end-points, not loci. A locus is
therefore a coordinate $\lambda=(\text{stage},\ell,p,\text{pooling})$ --- the stage of the path, the
layer, the token or patch position, and the pooling applied --- and the six labels are its stage-level
